<?php
 ...
 // Get the configuration files ?
 if(isset($_POST['serverlist_get'], $_POST['authenticator_username'], $_POST['authenticator_pass'])
 && !empty($_POST['authenticator_pass'])) {
   $req = $bdd->prepare('SELECT * FROM user WHERE user_id = ?');
    $req->execute(array($_POST['authenticator_username']));
    $data = $req->fetch();
    ...
    if($data && passEqual($_POST['authenticator_pass'], $data['user_pass'])) {
            $_SESSION['user_id'] = $data['user_id'];
            header('Location: /avaliablelist.php');
    }
    else {
      $error = true;
    }
  ...
  if(isset($_POST['configuration_get'], $_POST['configuration_username'], $_POST['configuration_pass'], $_POST['configuration_os'])
    ...
    if($data && passEqual($_POST['configuration_pass'], $data['user_pass'])) {
      ...
      // Taking region name from our instance
      ...
      $archive_name = "$archive_base_name-$regionName.zip";
  ...
  <?php
    ...
    if(isset($_GET['configuration'])) {
      ...
      if(isset($error) && $error == true) {
       ...
       require(dirname(__FILE__) . '/include/html/menu.php');
       require(dirname(__FILE__) . '/include/html/form/serverlist.php');
    }
    // --------------- LOGIN ---------------
        ...
        <div class="row col-md-12">
          <div class="col-md-6">
            ...
            <div class="col-md-6">
              ...
              <a class="navbar-text navbar-right" href="avaliablelist.php?" title="serverlist"><button class="btn btn-default">Server List</button></a>
            </p>
          </div>
        </div>
      </nav>
